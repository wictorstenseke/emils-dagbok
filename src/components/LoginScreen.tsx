...
export function LoginScreen({ onLogin }: Props) {
  ...
  function validate(input: number[]) {
    ...
    if (!profile) {
      // First run: create profile with defaults
    ...
    <div class="login-screen">
      <div class="login-card">
        ...
        <div class={`dots-row ${shake ? 'shake' : ''}`}>
          {[0, 1, 2, 3].map((i) => (
            <span key={i} class={`dot ${taps[i] !== undefined ? 'filled' : ''}`}>
              ●
            </span>
          ))}
        </div>
        ...
        <div class="tap-grid">
          {Array.from({ length: 25 }, (_, i) => i + 1).map((n) => (
            <button key={n} class="tap-cell" onClick={() => tap(n)}>
              {n}
            </button>
          ))}
        </div>

        <button class="backspace-btn" onClick={backspace} disabled={taps.length === 0}>
          ⌫
        </button>
      </div>
    </div>
